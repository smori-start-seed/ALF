...
# 1. Zellen erzeugen
# ---------------------------------------------------------
def create_zellen(num_zellen=11, hue=30, sat=0.6, bri=0.8):
    zellen = [
        Zelle(hue=hue, saturation=sat, brightness=bri)
        for _ in range(num_zellen)
    ]

    # einfache Nachbarschaft: alle mit allen
    for z in zellen:
        z.nachbarn = [n for n in zellen if n is not z]
...
# 2. Cluster erzeugen
# ---------------------------------------------------------
def create_cluster(num_cluster=5):
    cluster_liste = []
    for i in range(num_cluster):
        zellen = create_zellen()
        c = Cluster(zellen)
        cluster_liste.append(c)
    return cluster_liste

# ---------------------------------------------------------
# 3. Ringe erzeugen
# ---------------------------------------------------------
def create_ringe(num_ringe=3, cluster_pro_ring=5):
    ringe = []
    for i in range(num_ringe):
        cluster_liste = create_cluster(cluster_pro_ring)
        r = Ring(cluster_liste)
        ringe.append(r)
    return ringe
...
# ---------------------------------------------------------
def create_sphaere():
    ringe = create_ringe(num_ringe=3, cluster_pro_ring=5)
    return Sphere(ringe)

# ---------------------------------------------------------
# 5. Globale Simulation
# ---------------------------------------------------------
def run_simulation(steps=50, delay=0.1):
    sphere = create_sphaere()

    for step in range(steps):
        sphere.update()
        h, s, b = sphere.get_zustand()

        print(f"Step {step:03d} | Hue: {h:6.2f} | Sat: {s:4.2f} | Bri: {b:4.2f}")

        time.sleep(delay)

if __name__ == "__main__":
    run_simulation(steps=50, delay=0.05)
